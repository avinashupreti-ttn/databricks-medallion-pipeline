...
def apply_schema(spark, catalog: str, schema_name: str) -> None:
    for statement in render_schema_statements(catalog, schema_name):
        try:
            spark.sql(statement)
...
def _require_bronze(spark, catalog: str, schema_name: str) -> None:
    missing = []
    for entity, table in BRONZE_TABLES.items():
        full = _qualified(catalog, schema_name, table)
        if not spark.catalog.tableExists(f"{catalog}.{schema_name}.{table}"):
            missing.append(full)
    if missing:
        names = ", ".join(missing)
...
def _read_bronze(spark, catalog: str, schema_name: str, entity: str):
    table = _qualified(catalog, schema_name, BRONZE_TABLES[entity])
    try:
        return spark.table(f"{catalog}.{schema_name}.{BRONZE_TABLES[entity]}")
    except Exception as exc:
...
def build_silver_frames(spark, catalog: str, schema_name: str) -> dict:
    bronze = {
        entity: _read_bronze(spark, catalog, schema_name, entity)
...
def write_silver_tables(spark, catalog: str, schema_name: str, silver: dict) -> dict:
    counts = {}
...
def write_metrics_report(spark, catalog: str, schema_name: str, silver: dict) -> int:
    ...
    spark.sql(f"USE CATALOG `{catalog}`")
    spark.sql(f"USE SCHEMA `{schema_name}`")
    full = _qualified(catalog, schema_name, DQ_METRICS_TABLE)
    try:
        (
...
def create_silver_tables(spark, catalog: str, schema_name: str) -> dict:
    validate_identifier(catalog, "catalog")
    validate_identifier(schema_name, "schema")
    # Check Bronze before schema.sql. Applying schema first would CREATE empty
    # bronze_* tables and hide a missing ingestion.
    _require_bronze(spark, catalog, schema_name)
    apply_schema(spark, catalog, schema_name)
    silver = build_silver_frames(spark, catalog, schema_name)
    counts = write_silver_tables(spark, catalog, schema_name, silver)
    write_metrics_report(spark, catalog, schema_name, silver)
...
def run_main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        spark = get_spark()
        create_silver_tables(spark, config.catalog, config.schema)
